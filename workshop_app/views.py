...
from .models import (
    ...
    WorkshopCategory, WorkshopRating, Notification, ChatMessage, WorkshopSchedule,
    states
)
...
def index(request):
    """Landing Page : Show landing page if not logged in, otherwise redirect to dashboard"""
    user = request.user
...
def dashboard(request):
    ...
    user = request.user
    is_instructor_user = is_instructor(user)
    
    # Get user-specific data
    if is_instructor_user:
        workshops = Workshop.objects.filter(instructor=user)
        upcoming_workshops = workshops.filter(date__gte=timezone.now().date(), status=1).order_by('date')[:5]
        recent_workshops = workshops.order_by('-date')[:5]
        ...
        workshops = Workshop.objects.filter(coordinator=user)
        upcoming_workshops = workshops.filter(date__gte=timezone.now().date(), status=1).order_by('date')[:5]
        recent_workshops = workshops.order_by('-date')[:5]
        total_workshops = workshops.count()
        accepted_workshops = workshops.filter(status=1).count()
    ...
    # Get workshop statistics by month (last 12 months)
    ...
    for i in range(12):
        ...
        if is_instructor_user:
            count = workshops.filter(date__range=(month_start, month_end)).count()
        else:
            count = workshops.filter(date__range=(month_start, month_end)).count()
        
        months.append(month_start.strftime('%b %Y'))
        workshop_counts.append(count)
    
    months.reverse()
    workshop_counts.reverse()
...
def workshop_search(request):
    ...
    if query:
        workshops = workshops.filter(
            Q(workshop_type__name__icontains=query) |
            Q(coordinator__first_name__icontains=query) |
            Q(coordinator__last_name__icontains=query) |
            Q(coordinator__profile__institute__icontains=query) |
            Q(coordinator__profile__location__icontains=query)
        )
    ...
    if date_from and date_to:
        workshops = workshops.filter(date__range=(date_from, date_to))
    
    if difficulty:
        workshops = workshops.filter(workshop_type__difficulty_level=difficulty)
    
    if category:
        workshops = workshops.filter(workshop_type__category_id=category)
    
    # Pagination
    paginator = Paginator(workshops.order_by('date'), 20)
    page = request.GET.get('page')
    workshops = paginator.get_page(page)
    
    # Get filter options
    workshop_types = WorkshopType.objects.all()
    categories = WorkshopCategory.objects.all() if 'WorkshopCategory' in globals() else []
    states_list = states
    ...
    return render(request, 'workshop_app/workshop_search.html', context)
